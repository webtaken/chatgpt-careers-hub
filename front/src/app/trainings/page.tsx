...
async function fetchTrainings(): Promise<Post[]> {
  try {
    setBasePathToAPI();
...
export default async function TrainingsPage() {
  const posts = await fetchTrainings();
      ...
      <section className="max-w-6xl mx-auto px-4 py-10">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Curated learning resources and courses to level up your AI career.
          </p>
        </div>

        {!posts.length ? (
          <p className="text-muted-foreground">No trainings available.</p>
        ) : (
          <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => (
              <li key={post.id} className="group">
                <Link
                  href={`/trainings/${post.slug}`}
